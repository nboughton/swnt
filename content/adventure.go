...
// NewAdventure throws together a random adventure seed using the table available from Stars Without Number
func NewAdventure(worldTag string) Adventure {
	a := Adventure{
		Seed: adventureSeedTable.Roll(),
		Tag:  tagFields{Name: worldTag},
	}

	t, err := Tags.Find(worldTag)
	if err == nil {
...
var adventureSeedTable = roll.List{
	Items: []string{
		...
		"A Friend’s sibling is to be placed in a dangerous situation they’ve got no chance of surviving. The Friend takes their place at the last moment, and will almost certainly die unless the party aids them.",
		...
		"An Enemy who controls landing permits, oxygen rations, or some other important resource has a prejudice against one or more of the party members. He demands that they bring him a Thing from a dangerous Place before he’ll give them the goods.",
		...
		"A locked pretech stasis pod has been discovered by a Friend, along with directions to the hidden key-code that will open it. The Place where the keycode is hidden is now owned by an Enemy.",
		"A fierce schism has broken out in the local majority religion, and an Enemy is making a play to take control of the local hierarchy. A Friend is on the side that will lose badly if the Enemy succeeds, and needs a Thing to prove the other group’s error.",
		...
		"A Friend who is a skilled precognitive has just received a flash of an impending atrocity to be committed by an Enemy. He or she needs the party to help them steal the Thing that will prove the Enemy’s plans while dodging the assassins sent to eliminate the precog.",
		...
		"An Enemy and the group are suddenly trapped in a Place during an accident or Complication. They must work together to escape in time.",
		...
		"An Enemy has sold the party an expensive but worthlessly flawed piece of equipment before lighting out for the back country. He and his plunder are holed up at a remote Place.",
		"A concert of offworld music is being held in town, and a Friend is slated to be the star performer. Reactionary elements led by an Enemy plot to ruin the “corrupting noise” with sabotage that risks getting performers killed. Meanwhile, a crowd of ignorant offworlder fans have landed and are infuriating the locals.",
		...
		"An alien ambassador Friend is targeted by xeno-phobe Enemy assassins. Relations are so fragile that if the ambassador even realizes that humans are making a serious effort to kill him, the result may be war.",
		...
		"A Friend is importing offworld tech that threatens to completely replace the offerings of an Enemy businessman. The Enemy seeks to sabotage the friend’s stock, and thus ’prove’ its inferiority.",
